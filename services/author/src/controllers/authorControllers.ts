...
export const createBlog = async (req: authenticatedRequest, res: Response) => {
  try {
    ...
    const result =
      await sql`INSERT INTO blogs (title,description,blogcontent,image,category,author) VALUES (${title},${description},${blogcontent},${cloud.secure_url},${category},${req.user?._id}) RETURNING *`;
    res.json({
...
export const updateBlog = async (req: authenticatedRequest, res: Response) => {
  try {
    ...
    const updatedBlog =
      await sql`UPDATE blogs SET image=${imageUrl},  title = ${
        title || blog[0].title
      },
